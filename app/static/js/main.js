...
function init() {
    ...
    fetchData(`/api/v1/user/${tg.initDataUnsafe.user.id}`, "GET").then(response => {
        ...
        changeWeather(weather[indexWeater], false);
    })
...
function makeSplash(x, type) {
    ...
    TweenMax.fromTo(splash.node, speed, { strokeWidth: 2, y: yOffset, x: xOffset + 20 + x, opacity: 1, strokeDashoffset: splashLength }, { strokeWidth: 0, strokeDashoffset: - pathLength, opacity: 1, onComplete: onSplashComplete, onCompleteParams: [splash], ease: "slow(0.4, 0.1, false)" })
}
...
function changeWeather(weather_object, by_user) {
    if (weather_object.data) weather_object = weather.data;
    reset();
    localStorage.setItem('currentWeather', JSON.stringify(weather_object))
    ...
    weather_object.button.addClass('active');

    // windSpeed

    switch (weather_object.type) {
        case 'wind':
    ...
    if (by_user) {
        fetchData('/api/v1/user/', 'PATCH', {
            ...
            if (response.mood) {
                let indexWeater = weather.findIndex(function (mood) {
                    return mood.type === response.mood
...
async function fetchData(url, request_method, data) {
    if (!data){
